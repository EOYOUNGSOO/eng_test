...
class PhoneticFetchWorker(
    ...
    override suspend fun doWork(): Result =
        withContext(Dispatchers.IO) {
            ...
            val app =
                applicationContext.applicationContext as? EngTestApplication ?: run {
                    AppLogger.w(TAG, "Application is not EngTestApplication")
                    return@withContext Result.retry()
                }
            val c = app.appContainer
            val dao = c.database.wordDao()
            val repo = c.phoneticRepository
            val words = dao.getWordsWithEmptyPhonetic(WorkerConstants.PHONETIC_FETCH_BATCH_SIZE)
            if (words.isEmpty()) {
    ...
    private fun isWifiConnected(): Boolean {
        val cm =
            applicationContext.getSystemService(android.content.Context.CONNECTIVITY_SERVICE) as? ConnectivityManager
                ?: return false
